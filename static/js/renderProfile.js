import { escapeHtml } from "./render.js"; // you already have this

function formatGoDate(dateValue) {
...
export function renderProfileFromJSON(data) {
  ...
  const stats = data.stats || { postCount: 0, likesGiven: 0, dislikesGiven: 0 };
  ...
  const postsHtml = posts.length
    ? posts.map(p => `
      <li>
        <a href="#" class="profile-post-title"
          data-id="${p.ID}"
        ...
        <span class="profile-post-date">${escapeHtml(formatGoDate(p.CreatedAt))}</span>
      </li>
    `).join("")
    : `<li><em>No recent posts yet.</em></li>`;

  // In SPA: render it ONCE (cleaner + avoids duplicate IDs).
  const previewCats = categories
    ...
    <div class="profile-page container">
      <div class="profile-header">
        <div class="profile-avatar">
          <img src="https://api.dicebear.com/7.x/bottts/svg?seed=${encodeURIComponent(avatar)}" alt="User Avatar" />
        </div>
      ...
      <div class="modal-card">
        ...
        <form id="editProfileForm" class="auth-form">
          ...
          <div class="avatar-preview" style="margin:1rem 0; cursor:pointer;">
            <img id="avatarPreviewImg" src="" alt="Avatar preview" style="width:72px;height:72px;border-radius:50%;">
      ...
      <div class="modal-card post-card" id="modal-post-card">
        ...
        <div class="actions">
          <div class="reaction-group">
            <button class="like-btn" data-clicked="false"><span class="count" id="previewLikes">0</span></button>
            <button class="dislike-btn" data-clicked="false"><span class="count" id="previewDislikes">0</span></button>
            <button class="comment-toggle-btn"><span class="count" id="previewComments">0</span></button>
          </div>
        </div>
